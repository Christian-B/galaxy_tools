...
if __name__ == '__main__':
    ...
    help = "Path to file, where the output should be written. Defaults to "
    help += default_output_path
    parser.add_option("-o", "--output", dest="output_path",
                      help=help,
                      default=default_output_path)
    help = "Specifes the Lines numbers of Header and Data. "
    help += "All line numbers are 0 based so the top line is 0. "
    help += "If specified a header line can be below the first data line."
    line_group = OptionGroup(parser, "Line Options", help)
    help = "Line number of Header Line. Default is (-1) no header."
    line_group.add_option("--header", dest="header_line",
                          help=help,
                          default=default_header)
    help = "Line number of First line of Data. Default is "
    help += str(default_data_start)
    line_group.add_option("-d", "--data_start", dest="data_start",
                          help=help,
                          default=default_data_start)
    parser.add_option_group(line_group)
    help = "Specifes the Seperator to used to split columns. "
    help += "Defaults to tab. "
    help += "Only one Seperator option can be provided."

    seperator_group = OptionGroup(parser, "Seperator Options", help)
    seperator_group.add_option("-a", "--ascii_seperator",
                               dest="ascii_seperator",
                               help="ASCii code of seperator")
    seperator_group.add_option("-s", "--seperator", dest="seperator",
                               help="Column seperator as character.")
    ...
    if options.ascii_seperator:
        if options.seperator:
            error = "options ascii_seperator and seperator are "
