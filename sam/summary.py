...
def int_value(line_number, line, tag, value, min_value, max_value):
    try:
        ...
        print >> sys.stderr, "has an none ingeter", tag, "value"
        print >> sys.stderr, "Found", line
        sys.exit(1)
    if result < min_value:
        print >> sys.stderr, "Line", line_number
        print >> sys.stderr, "has a", tag, "value below", min_value
        print >> sys.stderr, "Found", line
        sys.exit(1)
    if result > max_value:
        print >> sys.stderr, "Line", line_number
        print >> sys.stderr, "has a", tag, "value above", max_value
        print >> sys.stderr, "Found", line
        sys.exit(1)
    return result
...
class SamHeader():
    ...
    def addAlignment(self, alignment, line_number, line):
        ...
        if len(self.reference_dictionary) > 0:
            if alignment.rname in self.reference_dictionary:
                ...
                print >> sys.stderr, "Aligment line", line_number
                print >> sys.stderr, "has an unexpected rname", alignment.rname
                print >> sys.stderr, "Found", line
                sys.exit(1)
    ...
    def process_header_line(self, line_number, line):
        if line.startswith("@HD"):
            ...
            program = Program(line_number, line, self)
            self.program_dictionary[program.id] = program
        elif line.startswith("@CO"):
            print "ignoring Comment(@CO) line"
        else:
            print >> sys.stderr, "Line", line_number, "has an unexpected @."
            print >> sys.stderr, "Found", line
            sys.exit(1)
        ...
        if line_number == 0:
            ...
            for part in parts[1:]:
                ...
                if tag == "VN:":
                    ...
                    if value in ["unknown", "unsorted", "queryname",
                                 "coordinate"]:
                        self.sorting_order = value
                    else:
                        print >> sys.stderr, "Line", line_number
                        print >> sys.stderr, "An unexpected '@SO:' ", value
                        print >> sys.stderr, "Found", line
                        sys.exit(1)
                elif tag == "CO:":
                    if value in ["none", "query", "reference"]:
                        ...
                        print >> sys.stderr, "Line", line_number
                        print >> sys.stderr, "An unexpected '@CO: ", value
                        print >> sys.stderr, "Found", line
                        sys.exit(1)
                else:
                    print >> sys.stderr, "Line", line_number
                    print >> sys.stderr, "has an unexpected tag", tag
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
            if not self.version:
        ...
        if line_number:
            ...
            for part in parts[1:]:
                ...
                if tag == "SN:":
                    ...
                    self.length = int_value(line_number, line, "LN:", value,
                                            1, 2**31-1)
                elif tag == "AS:":
                    self.assembly = value
                elif tag == "M5:":
                    ...
                    print >> sys.stderr, "Error in Line", line_number
                    print >> sys.stderr, "has an unexpected tag", tag
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
            if not self.name:
                print >> sys.stderr, "Error in Line", line_number
                print >> sys.stderr, "A sequence('@SQ') with no name('SN:')"
                print >> sys.stderr, "Found", line
                sys.exit(1)
            if not self.length:
                print >> sys.stderr, "Error in Line", line_number
                print >> sys.stderr, "A sequence('@SQ') with no length('LN:')"
                print >> sys.stderr, "Found", line
                sys.exit(1)
        else:
            self.name = line
    ...
    def summary_line(self, flag=False, position=False, mapq=False, cigar=False,
        ...
        if mapq:
            ...
            if mapq is True:
                ...
                total = sum(map(lambda x: x[0] * x[1],
                            self.mapq_counter.items()))
...
class Program:
    def __init__(self, line_number, line, header):
        self.id = None
        parts = line.strip().split("\t")
        for part in parts[1:]:
            tag = part[:3]
            ...
            if tag == "ID:":
                ...
                if header.program_dictionary[value]:
                    self.previous = header.program_dictionary[value]
                else:
                    print >> sys.stderr, "Program(@PG) line", line_number
                    print >> sys.stderr, "has an unused PP id:", value
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
            elif tag == "DS:":
...
class Alignment():
    def __init__(self, line_number, line):
        ...
        if len(parts) < 11:
            print >> sys.stderr, "Error in Line", line_number
            print >> sys.stderr, "has less than 11 columns."
            print >> sys.stderr, "Found", line
            sys.exit(1)
        self.qname = parts[0]
        ...
        self.rname = parts[2]
        self.position = int_value(line_number, line, "POS", parts[3],
                                  0, 2**31-1)
        ...
        if self.cigar != "*":
            ...
            if len(self.cigar) != sum(len(c) for c in cigars):
                print >> sys.stderr, "Unexpected cigar value in ", line_number
                print >> sys.stderr,  "found", self.cigar
                print >> sys.stderr, "Found", line
                sys.exit(1)
            self.cigar_long = ""
            for cigar in cigars:
        ...
        self.pnext_possition = int_value(line_number, line, "PNEXT",
                                         parts[7], 0, 2**31-1)
        self.tlen = int_value(line_number, line, "TLEN", parts[8],
                              -2**31+1, 2**31-1)
        ...
        for optional in parts[11:]:
            ...
            if tag in self.optional_fields:
                print >> sys.stderr, "Found duplicate optional tag", tag
                print >> sys.stderr, " in line", line_number
                print >> sys.stderr, "Found", line
                sys.exit(1)
            if the_type == "A":  # Printable character
                ...
                try:
                    ...
                    print >> sys.stderr, "Error in Line", line_number
                    print >> sys.stderr, "has an none float", tag, "value"
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
                ...
                try:
                    ...
                    print >> sys.stderr, "Error in Line", line_number
                    print >> sys.stderr, "has an none hex/bytearray", tag
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
                ...
                print >> sys.stderr, "Error in Line", line_number
                print >> sys.stderr, "Has an unexpected type", the_type
                print >> sys.stderr, "Found", line
                sys.exit(1)
...
def processOption(parameter, name, keys):
    if parameter.lower() == "ignore":
        ...
        if keys:
            ...
            print >> sys.stderr, "Count value not supported for", name
    else:
        print >> sys.stderr, "Unexpected value for", name, "parameter. "
        print >> sys.stderr, "Found", parameter
        sys.exit(1)
...
def summary(input_path,
        ...
        for line_number, line in enumerate(input_file):
            if line.startswith("@"):
                if in_header:
                    ...
                    print >> sys.stderr, "Error in Line", line_number
                    print >> sys.stderr, "has an unexpected @. "
                    print >> sys.stderr, "Wile no longer processing Header"
                    print >> sys.stderr, "Found", line
                    sys.exit(1)
            else:
                if in_header:
    ...
    with open(output_path, 'w') as output_file:
        ...
        output_file.write("\n")
        output_file.write(str(header.all_reference().summary_line(
                              flag=flag_show,
                              position=position_show,
                              mapq=mapq_show,
                              cigar=cigar_show,
    ...
    if flag_path:
        with open(flag_path, 'w') as output_file:
            ...
            output_file.write("\n")
            output_file.write(str(header.all_reference().summary_line(
                              flag=header.flag_keys())))
            output_file.write("\n")
            for key in header.reference_dictionary:
                reference = header.reference_dictionary[key]
                if reference.count > 0:
                    output_file.write(reference.summary_line(
                                      flag=header.flag_keys()))
                    output_file.write("\n")
    if mapq_path:
    ...
    if sequence_path:
        with open(sequence_path, 'w') as output_file:
            output_file.write(Reference.header_line(
                              sequence=header.sequence_keys()))
            output_file.write("\n")
            output_file.write(str(header.all_reference().summary_line(
                                  sequence=header.sequence_keys())))
            output_file.write("\n")
...
if __name__ == '__main__':
    ...
    help = "Path to general file, "
    help += "where any output not redirected below should be written. "
    help += "Defaults to summary.tsv"
    parser.add_option("-o", "--output", dest="output_path",
                      help=help,
                      default="summary.tsv")
    help = "Specifes what action should be taken "
    help += "with the data for this column. "
    help += "Legal values are 'ignore'(default), 'summary', 'count'. "
    help += "'ignore' or not provided will cause the column in this data not "
    help += "to be output. "
    help += "'summary' will cause summary columns to be added to the output "
    help += "file."
    help += "'count' will cause the counts of each value found to be added "
    help += "to the output file."
    help += "For example it will count how many of each of C, G, T, and A are "
    help += "found in the sequence."
    help += "Does not effect validation of lengths."

    data_group = OptionGroup(parser,
                             "Summary/Count individual Values Options ",
                             help)
    ...
    help = "Causes the position column to be output as requested. "
    help += "No Count position available"
    ...
    path_group = OptionGroup(parser, "Path to files to write Count details "
                             "to. "
                             "If provided will create a second output file "
                             "with the counts of the values found this column "
